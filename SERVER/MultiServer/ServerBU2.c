...
#define MAX_CLIENT (1000) /*Mex Clients*/
#define CONFIG_FILE ("./ServerConfigFile.txt")
#define CLOSE (-1)
...
static ServerStatus ActiveClientsCommunication(Server *_server, fd_set *_readFd, int *_activity);
...
ServerStatus RunServer(Server *_server)
{	
	ServerStatus status;
	int serverSocket = 0; 
	int activity = 0;
	...
	{	
		...
		status = ActiveClientsCommunication(_server, &readFd, &activity);
		if (status != ERR_SERVER_OK)
		{
...
static ServerStatus ActiveClientsCommunication(Server *_server, fd_set *_readFd, int *_activity)	
{
	...
	for (index = 0; index < _server->m_activeClients && currItr != endItr; index++)
	{
		...
		if(FD_ISSET(client->m_socket, _readFd))
		{
			...
			if (readBytes > 0)
			{
				...
				{
					status = send(client->m_socket, buffSend, wordLength, 0);
					if (status < 0)
					{
...
{	
	...
	begin = List_Begin(_server->m_inactiveClientsList);
	if (!begin)
	{
	...
	dest = ListItr_Splice(dest, begin, end);
	if (!dest)
	{
